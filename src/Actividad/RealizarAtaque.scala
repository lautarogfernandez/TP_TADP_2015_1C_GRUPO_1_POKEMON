...
case class RealizarAtaque(val ataque:AtaqueGenerico) extends Actividad {
   ...
   override def apply(pokemon: Pokemon) = {
      ...
      (ataque, ataque.tipo, pokemon, pokemon.especie.tipoPrincipal, pokemon.especie.tipoSecundario, pokemon.genero) match {
       ...
       case (ataque, _ : Dragon , pokemon, _ , _, _) => bajarPAGanarExperienciaYAplicarEfectoSecundario(pokemon, ataque, 80)
       ...
       case (ataque, tipoAtaque , pokemon, _ , tipoSecundarioPokemon, genero : Macho) if(tipoAtaque == tipoSecundarioPokemon) =>  bajarPAGanarExperienciaYAplicarEfectoSecundario(pokemon, ataque, 20)
                                                                                                                                         
       
       case (ataque, tipoAtaque , pokemon, _ , tipoSecundarioPokemon, _ : Hembra) if(tipoAtaque == tipoSecundarioPokemon) => bajarPAGanarExperienciaYAplicarEfectoSecundario(pokemon, ataque, 40)
